...
-- Add trigger for updated_at column (if the function doesn't exist, you can skip this part)
DO $$
BEGIN
...
DECLARE
  ...
  new_budget_amount DECIMAL(10,2);
BEGIN
  ...
  -- If adding credit (positive adjustment), add to current budget
  -- If removing credit (negative adjustment), subtract from current budget but don't go below 0
  new_budget_amount := GREATEST(current_budget + adjustment_amount, 0);
  
  -- Use UPSERT to insert or update budget
  -- This handles both cases: when budget record exists and when it doesn't
  INSERT INTO public.budgets (user_id, category_id, month_year, amount)
  VALUES (credit_user_id, target_category_id, target_month_year, new_budget_amount)
